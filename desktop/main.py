...
"""
    ...
    *********
    ...
    the serial port. When this happens. The code will:
        ...
        * Feed that data to the Steinhart-Hart class to (re)calibrate the module
...
from calibration.SteinhartHart import SteinHart
from calibration.Arduino import ArduinoSerial
from calibration.TSP01 import TSP01
...
if __name__ == '__main__':
    ...
    while True:
        ...
        calc_temp = sth.calculate_t(ntcResistance)
        ...
        with open(CSV_FILE, mode='a+') as csv_file:
            if not sth.calibrated:
                csv_file.writelines("%s, %s ,,,,,,,,,,\n" % (ntcResistance, probe_temp))
            else:
                csv_file.writelines("%s, %s, %s, %s, %s, %s, %s \n" %
